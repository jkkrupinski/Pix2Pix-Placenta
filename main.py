...
def load(image_file):
    ...
    input_image = tf.image.crop_to_bounding_box(
        input_image,
        int((input_size - 256) / 2),
        int((input_size - IMG_WIDTH) / 2),
        IMG_WIDTH,
        IMG_WIDTH,
    )
    real_image = tf.image.crop_to_bounding_box(
        real_image,
        int((real_size - 256) / 2),
        int((real_size - IMG_WIDTH) / 2),
        IMG_WIDTH,
        IMG_WIDTH,
    )
...
def load_single(image_file):
    ...
    # Crop
    input_image = tf.image.crop_to_bounding_box(
        input_image, 50, 0, IMG_WIDTH, IMG_WIDTH
    )
...
def test_big_picture():
    ...
    for i in range(int(fit_in_width)):
        for j in range(int(fit_in_height)):
            ...
            cropped_image = tf.image.crop_to_bounding_box(
                input_image, i * IMG_HEIGHT, j * IMG_WIDTH, IMG_HEIGHT, IMG_WIDTH
            )
    ...
    plt.imsave("stiched.png", buffer_image)
...
def test_scaled_picture():
    ...
    input_image = tf.image.crop_to_bounding_box(
        input_image,
        int((width - new_size) / 2),
        int((height - new_size) / 2),
        new_size,
        new_size,
    )
